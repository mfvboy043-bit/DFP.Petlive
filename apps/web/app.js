...
renderCoordinator.register(
  ...
  () => {
    alertCountBtn.textContent = t("noAlerts");
  }
);
renderCoordinator.register(
  "emergency",
...
const googleDriveAuth = PetLiveWeb.auth?.googleDrive || null;
...
function paintCloudChrome() {
  ...
  if (settingsEmail) {
    if (session.signedIn && session.profile?.email) {
  ...
  if (settingsLogin) settingsLogin.hidden = Boolean(session.signedIn);
  if (settingsSync) settingsSync.hidden = !session.signedIn;
  if (settingsLogout) settingsLogout.hidden = !session.signedIn;
}
